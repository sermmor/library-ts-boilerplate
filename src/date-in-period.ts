...
const splitDate = (stringDate: string): number[] => {
  const separatorDate: string = stringDate.includes(separatorDateTypeLine)
    ? separatorDateTypeLine
    : separatorDateTypeSlash;
...
const isMonthDayInPeriodWithSameYear = (
  [monthInput, dayInput]: number[],
  [monthBegin, dayBegin]: number[],
  [monthEnd, dayEnd]: number[]
): boolean => {
...
const isMonthDayInPeriodYearBeggining = (
  [monthInput, dayInput]: number[],
  [monthBegin, dayBegin]: number[]
): boolean => {
...
const isAValidPeriod = (
  [yearBegin, monthBegin, dayBegin]: number[],
  [yearEnd, monthEnd, dayEnd]: number[]
): boolean => {
 ...
 */
export const validateDateInPeriod = (
  dateInput: string,
